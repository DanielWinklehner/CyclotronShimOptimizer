...
Ported from the gmsh-OCC components in ``components_old.py``. Only the OCC/tet
path is provided here; the radia-native segmented builders were intentionally
dropped (they can be reintroduced later if ever needed).
...
from geometry.components import MagnetizedComponent, RadiaMaterial
...
def build_wedge(
    *,
    ...
    return MagnetizedComponent.from_gmsh_occ(
        occ, model_name=model_name, mesh_size_min=1.0, mesh_size_max=max_mesh_size,
...
def build_pole(
    *,
    ...
    return MagnetizedComponent.from_gmsh_occ(
        occ, model_name=model_name, mesh_size_min=10.0, mesh_size_max=max_mesh_size,
        comm=comm, material=material, symmetries=symmetries, color=color,
        apply_mat=apply_mat, apply_color=apply_color, apply_sym=apply_sym,
    )
