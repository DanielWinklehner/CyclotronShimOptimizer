"""Legacy full-analysis workflow (the former main.py main() function).

``run()`` performs the complete config-driven flow: load config -> (optional
shim optimization) -> build + solve -> isochronism -> saves -> plots/viewer.
Kept as the compatibility path behind the thin main.py CLI; new project
scripts should prefer the Session/CyclotronModel facade (see
cyclotron_optimizer.session), which exposes the same steps a la carte.

The MKL/CONDA_PREFIX/radia-before-mpi4py environment handling lives in the
package __init__ -- importing cyclotron_optimizer (or this module through it)
is enough.
"""
import gmsh  # noqa: F401  (loaded early: OpenMP runtime ordering)
import radia as rad

import time
from cyclotron_optimizer.visualization.field_comparison import compare_fields
...
def run(rank: int = 0, comm=None, verbosity: int = 1, run_optimization: bool = False, test_geometry: bool = False,
        conf: str = 'config.yml'):
    """
...
def cli(argv=None):
    """Console entry point (also reachable as ``python main.py`` at repo root)."""
    import argparse
    ...
    args = parser.parse_args(argv)  # --help exits BEFORE MPI is initialized

    from mpi4py import MPI  # radia imported above -> ordering satisfied
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()

    # Adopt the mpi4py-initialized MPI environment in radia ('in'), so the
    # relaxation is rank-distributed instead of every rank solving the full
    # problem (which makes mpiexec -n N run N x slower on one GPU).
    try:
        rad.UtiMPI('in')
    except Exception:
        pass

    results = run(rank=rank,
                  comm=comm,
...
if __name__ == '__main__':
    # Legacy CLI: [mpiexec -n N] python -m cyclotron_optimizer.api
    #             [--config <yml>] [--optimize] [--geo_test] [--verbosity N]
    cli()
